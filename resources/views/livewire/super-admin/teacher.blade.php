    ...
    <x-view-modal :show="$showViewModal" :title="$viewModalTitle" closeAction="closeViewModal">
        @if (!empty($viewData))
            <div class="space-y-5 text-left text-sm text-gray-700">
                    ...
                    @else
                        <div class="w-16 h-16 rounded-full bg-purple-100 flex items-center justify-center">
                            <span class="text-xl font-bold text-purple-600">
                                {{ strtoupper(substr($viewData['user']?->name ?? 'T', 0, 1)) }}
                            </span>
                        </div>
                    @endif
                    <div>
                ...
                <div>
                    ...
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <dd class="font-medium capitalize">{{ $viewData['user']?->gender ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs text-gray-400">Date of Birth</dt>
                            <dd class="font-medium">{{ $viewData['user']?->dob ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs text-gray-400">Qualification</dt>
                            <dd class="font-medium">{{ $viewData['detail']?->qualification ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs text-gray-400">Emergency Contact</dt>
                            <dd class="font-medium">{{ $viewData['detail']?->emergency_contact ?? '—' }}</dd>
                        </div>
                    </dl>
                </div>
                ...
                {{-- Professional --}}
                <div>
                    <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Professional
                        Information</h4>
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            <dt class="text-xs text-gray-400">Employee ID</dt>
                            <dd class="font-medium font-mono">{{ $viewData['detail']?->employee_id ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs text-gray-400">Date of Joining</dt>
                            <dd class="font-medium">{{ $viewData['detail']?->date_of_joining ?? '—' }}</dd>
                        </div>
                        <div class="sm:col-span-2">
                ...
                <div>
                    ...
                    <dl class="grid grid-cols-1 gap-3">
                        <div>
                            ...
                            <dd class="font-medium">{{ $viewData['detail']?->address ?? '—' }}</dd>
                        </div>
