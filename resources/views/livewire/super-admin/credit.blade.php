<div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white border-b border-gray-200 px-4 sm:px-6 py-4 sm:py-5 sticky top-0 z-50">
        ...
        {{-- Analytics cards row (visible on smaller screens) --}}
        <div class="xl:hidden mt-4">
            <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-7 gap-2">
                @foreach([
                    ['label' => 'Total',      'val' => $analytics['total'],               'color' => 'text-gray-800'],
                    ['label' => 'Pending',     'val' => $analytics['pending'],             'color' => 'text-amber-500'],
                    ['label' => 'Processing',  'val' => $analytics['processing'],          'color' => 'text-blue-600'],
                    ['label' => 'Approved',    'val' => $analytics['approved'],            'color' => 'text-emerald-600'],
                    ['label' => 'Denied',      'val' => $analytics['denied'],              'color' => 'text-red-500'],
                    ['label' => 'Active',      'val' => $analytics['active_credits'],      'color' => 'text-indigo-600'],
                    ['label' => 'Leased',      'val' => '₹'.number_format($analytics['total_amount_leased'],0), 'color' => 'text-gray-800'],
                ] as $stat)
                    <div class="bg-gray-50 rounded-lg px-2 py-1.5 text-center">
                        <p class="text-[10px] text-gray-400">{{ $stat['label'] }}</p>
                        <p class="text-sm font-bold {{ $stat['color'] }}">{{ $stat['val'] }}</p>
                    </div>
                @endforeach
            </div>
        </div>

        {{-- Filters (shown in credit tab) --}}
        @if ($activeTab === 'credit')
    ...
    <div class="p-4 sm:p-6 space-y-5">
        ...
        {{-- ── Analytics Cards (summary row) ── --}}
        <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-4 gap-3">
            <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                <p class="text-xs text-gray-400 font-medium mb-1">Total Amount Leased</p>
                <p class="text-xl font-bold text-indigo-700">₹{{ number_format($analytics['total_amount_leased'], 0) }}</p>
            </div>
            <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                <p class="text-xs text-gray-400 font-medium mb-1">Total Penalties</p>
                <p class="text-xl font-bold text-red-600">₹{{ number_format($analytics['total_penalties'], 0) }}</p>
            </div>
            <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                <p class="text-xs text-gray-400 font-medium mb-1">Amount to Collect</p>
                <p class="text-xl font-bold text-emerald-700">₹{{ number_format($analytics['total_to_collect'], 0) }}</p>
            </div>
            <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                <p class="text-xs text-gray-400 font-medium mb-1">Active Credit Schools</p>
                <p class="text-xl font-bold text-blue-700">{{ $analytics['active_credits'] }}</p>
            </div>
        </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead class="bg-gray-50 border-b border-gray-200">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500">School</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500">Heading / Reason</th>
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <p class="text-[10px] text-gray-400 truncate">{{ $org->email }}</p>
                                                <p class="text-[10px] text-gray-400">{{ $org->mobile_number ?? '' }}</p>
                                            </div>
                                        </div>
                                    </td>
                            ...
                            <div class="bg-white border border-gray-200 rounded-xl px-5 py-4 flex items-center gap-4">
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-center gap-3 mt-1">
                                        @if ($policy->image)
                                            <span class="text-[10px] text-blue-500">Image</span>
                                        @endif
                                        @if ($policy->document)
                                            <span class="text-[10px] text-red-500">PDF</span>
                                        @endif
                                        @if ($policy->link)
                                            <span class="text-[10px] text-indigo-500">Link</span>
                                        @endif
                                    </div>
                                </div>
    ...
    <x-view-modal :show="$showViewModal && $selectedQuery !== null"
        ...
        @if ($selectedQuery)
            ...
            {{-- School info --}}
            <div class="flex items-center gap-3 mb-4 bg-gray-50 rounded-xl p-3">
                @if ($org->logo)
                    <img src="{{ $org->logo }}" class="w-10 h-10 rounded-full object-cover border border-gray-200 flex-shrink-0">
                @else
                    <div class="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center flex-shrink-0">
                        <span class="text-sm font-bold text-indigo-600">{{ strtoupper(substr($org->name,0,1)) }}</span>
                    </div>
                @endif
                <div class="text-left min-w-0">
                    <p class="text-sm font-semibold text-gray-800">{{ $org->name }}</p>
                    <p class="text-xs text-gray-400">{{ $org->email }} · {{ $org->mobile_number ?? '' }}</p>
                    <p class="text-xs text-gray-400">{{ $org->address ?? '' }}</p>
                </div>
            </div>

            {{-- Stats strip --}}
            <div class="flex flex-wrap gap-3 text-xs text-gray-500 mb-4 bg-gray-50 rounded-xl px-4 py-2">
                <span>Students: <strong class="text-gray-800">{{ $org->total_students ?? 0 }}</strong></span>
                <span class="border-l border-gray-300 pl-3">Teachers: <strong class="text-gray-800">{{ $org->total_teachers ?? 0 }}</strong></span>
            </div>
        ...
        <x-slot:footer>
            <div class="flex items-center gap-2">
                @if ($selectedQuery && $selectedQuery->status !== 'approved')
                    <button wire:click="openApproveModal({{ $selectedQuery->id }}); closeViewModal()"
                        class="px-4 py-2 text-sm font-semibold bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors">
                        Approve
                    </button>
                @endif
        ...
        <div class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh] pointer-events-auto">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
                    <h3 class="text-base font-semibold text-gray-900">Approve Credit Query</h3>
                    <button wire:click="closeApproveModal" class="text-gray-400 hover:text-gray-600 transition-colors">
                ...
                <div class="flex-1 overflow-y-auto px-6 py-4 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea wire:model="approveRemark" rows="3" placeholder="Add approval notes..."
                            class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg resize-none focus:ring-2 focus:ring-blue-500"></textarea>
                    </div>
                </div>
                <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 flex-shrink-0">
                    ...
                    <button wire:click="approveQuery"
                        class="px-5 py-2 text-sm font-semibold bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors">
                        Pay Now &amp; Approve
                    </button>
                </div>
            </div>
    ...
    @if ($showStatusModal)
        <div class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[90vh] pointer-events-auto">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
                    <h3 class="text-base font-semibold text-gray-900">Update Status</h3>
                    <button wire:click="closeStatusModal" class="text-gray-400 hover:text-gray-600 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                        </svg>
                    </button>
                ...
                <div class="flex-1 overflow-y-auto px-6 py-4 space-y-4">
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            @foreach(['pending' => ['amber', 'Pending'], 'processing' => ['blue', 'Processing'], 'approved' => ['emerald', 'Approved'], 'denied' => ['red', 'Denied']] as $val => $meta)
                                <button wire:click="$set('newStatus', '{{ $val }}')"
                                    class="py-2 px-3 text-sm font-medium rounded-lg border-2 transition-colors
                                        {{ $newStatus === $val
                                            ? 'border-'.$meta[0].'-500 bg-'.$meta[0].'-50 text-'.$meta[0].'-700'
                    ...
                    <div>
                        ...
                        <textarea wire:model="statusRemark" rows="4" placeholder="Add a remark for the school..."
                            class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg resize-none focus:ring-2 focus:ring-blue-500"></textarea>
                    </div>
                </div>
                <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 flex-shrink-0">
    ...
    @if ($showPolicyForm)
        <div class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh] pointer-events-auto">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
                    <h3 class="text-base font-semibold text-gray-900">
                        {{ $editPolicyId ? 'Edit Policy' : 'Add New Policy' }}
                    </h3>
                    <button wire:click="closePolicyForm" class="text-gray-400 hover:text-gray-600 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                        </svg>
                    </button>
                ...
                <div class="flex-1 overflow-y-auto px-6 py-4 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea wire:model="policyContent" rows="6"
                            placeholder="Write policy content here (terms and conditions style)..."
                            class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg resize-y focus:ring-2 focus:ring-blue-500"></textarea>
                        ...
                        <div>
                            ...
                            <input wire:model="policyDocument" type="file" accept=".pdf,.doc,.docx"
                                class="w-full text-sm text-gray-600 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg
                                       ...
                                       hover:file:bg-red-100 border border-gray-300 rounded-lg"/>
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
                ...
                <div class="flex gap-3">
                    <button wire:click="cancelDeleteQuery"
                        class="flex-1 px-4 py-2 text-sm text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50">Cancel</button>
                    <button wire:click="executeDeleteQuery"
                        class="flex-1 px-4 py-2 text-sm font-semibold text-white bg-red-600 hover:bg-red-700 rounded-lg">Delete</button>
                </div>
            </div>
        </div>
    @endif
